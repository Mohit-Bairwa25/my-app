...
// ROUTE 2 : Add New Note using: POST "/api/notes/addnote". Login required
        router.post('/addnote', fetchUser, [
            body('title','Enter a Valid Title of Minimun Length 3').isLength({min:3}),
            body('description','Enter a Valid Description of Minimun Length 5').isLength({min:5}),
        ],async (req, res)=>{
                try {
...
router.delete('/deletenote/:id', fetchUser, async (req, res)=>{
    try {
        ...
        // Allow termination of Note if User owns this note
        if(note.user.toString() !== req.user.id){
            return res.status(401).send("Not Allowed");
        }
